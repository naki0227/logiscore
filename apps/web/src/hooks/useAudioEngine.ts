    ...
    async (
      ...
      const getSynth = (program: number) => {
        const type =
          program === 48
            ? "strings"
            : program === 42 || program === 45
              ? "cello"
              : program === 60 || program === 56
                ? "brass"
                : program === 71 || program === 73
                  ? "woodwind"
                  : program === 46 || program === 10
                    ? "keyboard"
                    : program === 47 || program === 14
                      ? "percussion"
                      : "piano";
      ...
      for (let i = 0; i < ticks.length; i++) {
        ...
        for (let j = 0; j < tickNotes.length; j++) {
          ...
          const duration = isBass
            ? autoTickInterval * 0.8
            : autoTickInterval * 0.6;
          ...
          if (onNote) {
            setTimeout(
              () => onNote(finalMidiNote, vel, duration, isBass),
              strumDelay * 1000,
            );
          }
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, autoTickInterval * 1000),
        );
      }
